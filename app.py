...
SCAN_WORKERS = 4          # خُفّض من 6 إلى 4 — رفع TOP_N إلى 20 مع تعدد المزوّدين
                          # كان يسبب ضغط طلبات كبير دفعة واحدة على OKX (حدود معدل)
...
def _call(op,*args):
    ...
    # HTTP 400 غالباً يعني "رمز غير موجود على هذا المزوّد" (وليس حظراً) —
    # يحدث لعملات مدرجة على مزوّد واحد فقط (نادرة) وليس مؤشر عطل عام،
    # فنسجّله كتحذير خفيف بدل خطأ حرج لتفادي إيحاء مضلّل بحظر شامل.
    lvl = log.warning if "400" in str(last_err) else log.error
    lvl(f"{op}: فشلت كل مصادر البيانات (OKX + Binance وكل البروكسيات){sym_ctx} — آخر خطأ: {last_err}")
