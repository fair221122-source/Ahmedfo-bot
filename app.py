...
RR        = 4           # نسبة المخاطرة 1:4 (إصلاح #32 — مطابقة لمثال استراتيجية Break & Retest)
...
function renderSignals(signals){
  ...
  w.innerHTML=signals.slice(0,2).map(s=>{
    ...
    <div class="scard ${s.direction}">
      ...
      <div class="steps">
        <span class="step s4h">📍 ${s.trend_4h} @ ${s.zone_level} (${s.zone_touches} لمسات)</span>
        <span class="step s1h">💥 كسر + Pull Back (دقة ${s.retest_precision}%)</span>
        <span class="step s15m">🎯 دخول ${s.entry}</span>
      </div>
      <div class="sgrid">
        ...
        <div class="si2"><div class="lb">🏆 هدف (1:${s.rr})</div><div class="vl vtp">${s.tp}</div></div>
        ...
        <div class="si2"><div class="lb">📈 حجم الكسر</div><div class="vl">×${s.break_vol_ratio||1}</div></div>
        <div class="si2"><div class="lb">⏱ الجلسة</div><div class="vl">${s.session}</div></div>
        <div class="si2"><div class="lb">RSI 1H</div><div class="vl">${s.rsi_15m}</div></div>
      ...
      <div class="tags" style="margin-bottom:8px">
        <span class="tag tg-gold">📍 منطقة ${s.trend_4h} مؤكدة (إلزامي)</span>
        <span class="tag tg-grn">💥 كسر + Pull Back (إلزامي)</span>
        <span class="tag tg-pur">🔢 ${s.zone_touches} لمسات</span>
        ${s.in_kz?'<span class="tag tg-grn">⚡ Kill Zone</span>':''}
        ${s.break_vol_ratio>=1.5?'<span class="tag tg-blue">📈 كسر بحجم قوي</span>':''}
      </div>
...
# ═══════════════════════════════════════════
#  إصلاح #32 (بطلب صريح): استراتيجية Support & Resistance + Break & Retest
#  الكلاسيكية — استبدال كامل لخط أنابيب ICT السابق (سحب سيولة/FVG/OB/
#  CHoCH/BOS/CISD). كل الخطوات الأربع على فريم الساعة فقط كما حُدِّد:
#  1) تحديد منطقتي دعم/مقاومة، كل منطقة لُمست وارتد عنها السعر مرتين
#     على الأقل. 2) كسر المنطقة (إغلاق يتجاوزها). 3) Pull Back — إعادة
#     اختبار المنطقة المكسورة دون إغلاق يُبطلها بالعكس. 4) الدخول بعد
#     الـ Pull Back مباشرة. الدوال القديمة (find_fvg, find_ob, إلخ) بقيت
#     معرَّفة بالملف دون استخدام — لا خطر من إبقائها، فقط كود غير مُستدعى.
# ═══════════════════════════════════════════

def find_sr_zones(c1h, lookback=120, min_touches=2, tol_pct=0.004):
    """
    يحدد أحدث منطقة مقاومة وأحدث منطقة دعم مؤكدتين على فريم الساعة —
    كل منطقة لامسها السعر وارتد عنها (قمة/قاع محوري) مرتين على الأقل
    (min_touches)، والقمم/القيعان المتقاربة سعرياً (ضمن tol_pct) تُجمَّع
    كمنطقة واحدة بدل مستويات منفصلة وهمية. يُرجع (resistance, support)،
    كل عنصر إما dict {"level","touches","last_idx"} (last_idx فهرس مطلق
    داخل c1h) أو None لو لم يتحقق شرط اللمستين.
    """
    if len(c1h) < 30: return None, None
    recent = c1h[-lookback:] if len(c1h) > lookback else c1h
    offset = len(c1h) - len(recent)
    piv_h=[]; piv_l=[]
    for i in range(2, len(recent)-2):
        cv=recent[i]
        if cv['h']>recent[i-1]['h'] and cv['h']>recent[i-2]['h'] and \
           cv['h']>recent[i+1]['h'] and cv['h']>recent[i+2]['h']:
            piv_h.append((cv['h'], i+offset))
        if cv['l']<recent[i-1]['l'] and cv['l']<recent[i-2]['l'] and \
           cv['l']<recent[i+1]['l'] and cv['l']<recent[i+2]['l']:
            piv_l.append((cv['l'], i+offset))
    ...
    def cluster(pivots):
        ...
        zones=[]; cur=[pivots[0]]
        for p in pivots[1:]:
            if abs(p[0]-cur[-1][0]) <= cur[-1][0]*tol_pct: cur.append(p)
            else: zones.append(cur); cur=[p]
        zones.append(cur)
        return [{"level":sum(x[0] for x in z)/len(z), "touches":len(z),
                 "last_idx":max(x[1] for x in z)} for z in zones]

    res_zones=[z for z in cluster(piv_h) if z["touches"]>=min_touches]
    sup_zones=[z for z in cluster(piv_l) if z["touches"]>=min_touches]
    resistance=max(res_zones, key=lambda z:z["last_idx"]) if res_zones else None
    support   =max(sup_zones, key=lambda z:z["last_idx"]) if sup_zones else None
    return resistance, support

def find_breakout_retest(c1h, zone, direction):
    """
    يتحقق من تسلسل الكسر ثم إعادة الاختبار (خطوتا 2 و3) بعد آخر لمسة
    مؤكِّدة للمنطقة (zone["last_idx"]). يُرجع
    (breakout_ok, retest_ok, breakout_idx, swing_extreme, vol_ratio_at_break).
    swing_extreme: أدنى قاع (BUY) أو أعلى قمة (SELL) منذ الكسر — أساس
    حساب وقف الخسارة لاحقاً ("تحت آخر قاع"/"فوق آخر قمة" كما طُلب).
    إغلاق يعيد كسر المنطقة بعكس اتجاه الصفقة بعد الكسر الأول يُبطل الإشارة
    فوراً (يعني كسراً كاذباً/فخ سيولة، لا Pull Back حقيقياً).
    """
    if not zone: return False,False,None,None,1.0
    level=zone["level"]; start=zone["last_idx"]
    after=list(enumerate(c1h))[start+1:]
    if len(after) < 3: return False,False,None,None,1.0

    breakout_idx=None; break_vol=1.0
    for idx,cv in after:
        if direction=="BUY" and cv['c']>level:
            breakout_idx=idx
            break_vol=vol_ratio(c1h[:idx+1],20)
            break
        if direction=="SELL" and cv['c']<level:
            breakout_idx=idx
            break_vol=vol_ratio(c1h[:idx+1],20)
            break
    if breakout_idx is None: return False,False,None,None,1.0
    ...
    for cv in post:
        ...
        if direction=="BUY":
            if cv['c'] < level-tol: return True,False,breakout_idx,None,break_vol   # كسر كاذب — إبطال
            if touched: retested=True
        else:
            if cv['c'] > level+tol: return True,False,breakout_idx,None,break_vol
            if touched: retested=True
    if not retested: return True,False,breakout_idx,None,break_vol

    swing_extreme = min(cv['l'] for cv in post) if direction=="BUY" else max(cv['h'] for cv in post)
    return True,True,breakout_idx,swing_extreme,break_vol
...
def analyze(sym,btc_c,learned=None,debug=None,cvd_precomputed=None):
    ...
    try:
        ...
        if len(c1h)<60:
            return reject("بيانات تاريخية غير كافية على 1H")
        cur=c1h[-1]['c']

        # ─── الخطوة 1: منطقتا دعم ومقاومة مؤكدتان (لمستان+) ─────────────
        resistance,support=find_sr_zones(c1h,120,2,0.004)
        if not resistance and not support:
            return reject("لا توجد مناطق دعم/مقاومة مؤكدة بلمستين على الأقل على 1H")

        # ─── الخطوات 2+3: كسر + Pull Back — نفحص الجهتين ونختار الأحدث ──
        buy_break,buy_retest,buy_bidx,buy_swing,buy_vr = find_breakout_retest(c1h,resistance,"BUY")
        sell_break,sell_retest,sell_bidx,sell_swing,sell_vr = find_breakout_retest(c1h,support,"SELL")

        buy_ready  = buy_retest  and buy_swing  is not None
        sell_ready = sell_retest and sell_swing is not None
        if not buy_ready and not sell_ready:
            if buy_break or sell_break:
                return reject("كسر تحقق لكن لم تكتمل إعادة الاختبار (Pull Back) بعد")
            return reject("لا يوجد كسر مؤكد لأي منطقة دعم/مقاومة بعد")

        if buy_ready and sell_ready:
            # الأحدث زمنياً هو الأكثر صلة الآن
            if buy_bidx >= sell_bidx: sell_ready=False
            else: buy_ready=False

        if buy_ready:
            direction="BUY"; zone=resistance; swing_extreme=buy_swing; break_vr=buy_vr
        else:
            direction="SELL"; zone=support; swing_extreme=sell_swing; break_vr=sell_vr

        # ─── الخطوة 4: الدخول فوراً بعد الـ Pull Back ────────────────────
        entry=round(cur,6)

        # وقف الخسارة تحت آخر قاع (شراء) / فوق آخر قمة (بيع) — بهامش أمان
        # بسيط (0.15%) لتفادي الخروج بفتيل عابر بالضبط على المستوى، وهذا
        # التفصيل العملي الوحيد المُضاف فوق النص الأصلي للاستراتيجية.
        buf=swing_extreme*0.0015
        if direction=="BUY":
            sl=round(swing_extreme-buf,6)
            risk=entry-sl
        ...
        if risk<=0: return reject("حساب الوقف/المخاطرة غير منطقي (وقف على نفس جهة الدخول)")
        rr_actual=round(abs(tp-entry)/abs(sl-entry),1) if abs(sl-entry)>0 else RR

        # ─── CVD (إلزامي، من "بقية الأشياء" التي بقيت كما هي) ───────────
        ...
        rsi_1h=rsi_calc(c1h)
        if direction=="BUY"  and rsi_1h>75: return reject("RSI 1H مرتفع جداً (>75) لصفقة شراء بعد الكسر")
        if direction=="SELL" and rsi_1h<25: return reject("RSI 1H منخفض جداً (<25) لصفقة بيع بعد الكسر")
        ...
        retest_precision = max(0.0, 1-abs(entry-zone["level"])/max(zone["level"]*0.005,1e-9))
        score=0.0
        score+=min(20, zone["touches"]*7)                 # قوة المنطقة (عدد اللمسات)
        score+=round(retest_precision*20,1)                # دقة إعادة الاختبار (قرب الدخول من المستوى)
        ...
        sig_tmp={"session":sess["session"],"in_kz":sess["in_kz"],
                 "cvd_score":cvd_data["score"],"direction":direction,"symbol":sym}
        brain_bonus=0
        ...
        # ─── تفسير لماذا ─────────────────────
        zone_ar="مقاومة" if direction=="BUY" else "دعم"
        reasons=[]
        reasons.append(f"1H: منطقة {zone_ar} عند {fmt(zone['level'])} ({zone['touches']} لمسات مؤكدة)")
        reasons.append(f"1H: كسر مؤكد + Pull Back ناجح (دقة إعادة اختبار {retest_precision*100:.0f}%)")
        ...
        return {
            ...
            "rr"           :rr_actual,
            "score"        :score,
            "rsi_15m"      :rsi_1h,           # نفس المفتاح القديم (rsi_15m) للتوافق مع brain.py، والقيمة الآن RSI 1H
            "trend_4h"     :zone_ar,          # يُستخدم بالواجهة لعرض نوع المنطقة المكسورة
            "trend_1h"     :direction,
            "session"      :sess["session"],
            "in_kz"        :sess["in_kz"],
            "zone_level"   :fmt(zone["level"]),
            "zone_touches" :zone["touches"],
            "retest_precision":round(retest_precision*100,1),
            "break_vol_ratio":break_vr,
...
def send_tg(sig,raw=False):
    if raw: msg=sig
        ...
        msg=f"""
🚀 *CryptoBot Pro — إشارة جديدة (Break & Retest)*
{'─'*22}
💰 *{s['symbol']}* | {d}
{'─'*22}
📍 دخول: `{s['entry']}`
...
📐 R:R: *1:{s['rr']}*
{'─'*22}
📍 منطقة {s['trend_4h']} @ {s.get('zone_level')} ({s.get('zone_touches')} لمسات مؤكدة)
💥 كسر + Pull Back — دقة إعادة الاختبار: {s.get('retest_precision',0)}% | حجم الكسر: ×{s.get('break_vol_ratio',1)}
📊 RSI 1H: {s['rsi_15m']}
